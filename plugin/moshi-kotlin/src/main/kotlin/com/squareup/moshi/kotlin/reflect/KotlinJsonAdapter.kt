...
internal class KotlinJsonAdapter<T>(
  ...
  override fun toJson(writer: JsonWriter, value: T?) {
    ...
    val bindings = allBindings.filterNotNull().map { it to it.get(value) }.filter { (_, v) ->
      !(v == null || v is Collection<*> && v.isEmpty() || v is Map<*, *> && v.isEmpty())
    }
    if (bindings.isNotEmpty()) {
      writer.beginObject()
      for ((binding, v) in bindings) {
        writer.name(binding.jsonName)
        binding.adapter.toJson(writer, v)
      }
      writer.endObject()
    } else
      writer.nullValue()
...
public class KotlinJsonAdapterFactory : JsonAdapter.Factory {
  override fun create(type: Type, annotations: MutableSet<out Annotation>, moshi: Moshi):
    ...
    for (property in rawTypeKotlin.memberProperties) {
      ...
      require(parameter == null || parameter.type.isSubtypeOf(property.returnType)) {
...
//          }
    ...
    return KotlinJsonAdapter(constructor, bindings, nonIgnoredBindings, options)
  }
}
